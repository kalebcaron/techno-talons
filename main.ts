namespace SpriteKind {
    export const villain = SpriteKind.create()
    export const accessory = SpriteKind.create()
}
/**
 * Shared variables
 */
/**
 * Level 1 variables
 */
/**
 * Level 2 variables
 */
controller.B.onEvent(ControllerButtonEvent.Pressed, function () {
    if (currentLevel == 2) {
        effects.blizzard.startScreenEffect(500)
    }
})
function level1 () {
    if (previousLevel == 0 && currentLevel == 1) {
        kalebSprite = sprites.create(kaleb, SpriteKind.Player)
        scene.setBackgroundImage(img`
            
            `)
        controller.moveSprite(kalebSprite)
        kalebSprite.setStayInScreen(true)
        music.play(music.createSong(hex`0078000408020106001c00010a006400f4016400000400000000000000000000000000000000024c0000000400012004000800012008000c0001200c001000011d14001800011d18001c00011d1c002000011d20002400021b2024002800012228002c0001252c003000012930003400041d202427`), music.PlaybackMode.LoopingInBackground)
    }
}
controller.A.onEvent(ControllerButtonEvent.Pressed, function () {
    if (currentLevel == 2) {
        projectile = sprites.createProjectileFromSprite(img`
            . . . . . . . . . . . . . . . . 
            . . . . . . 4 4 4 4 4 4 . . . . 
            . . . . . 4 4 4 4 e 4 4 4 . . . 
            . . . . . 4 4 4 4 e e 4 4 . . . 
            . . . . . 4 4 4 4 e 4 e 4 . . . 
            . . . . . 4 4 4 4 e 4 e 4 . . . 
            . . . . . 4 4 4 4 e 4 e 4 . . . 
            . . . . . 4 4 4 4 e 4 4 4 . . . 
            . . . . . 4 4 4 4 e 4 4 4 . . . 
            . . . . . 4 4 e e e 4 4 4 . . . 
            . . . . . 4 e e e e 4 4 4 . . . 
            . . . . . 4 e e e e 4 4 4 . . . 
            . . . . . 4 4 e e e 4 4 4 . . . 
            . . . . . . 4 4 4 4 4 4 . . . . 
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            `, myShip, 0, -150)
    }
})
function intro () {
    kalebSprite = sprites.create(kaleb, SpriteKind.Player)
    hunterSprite = sprites.create(hunter, SpriteKind.Player)
    nandaniSprite = sprites.create(nandani, SpriteKind.Player)
    momoSprite = sprites.create(moselle, SpriteKind.Player)
    haydenSprite = sprites.create(hayden, SpriteKind.Player)
    evanSprite = sprites.create(evan, SpriteKind.Player)
    kalebSprite.setPosition(30, 25)
    nandaniSprite.setPosition(70, 25)
    hunterSprite.setPosition(110, 25)
    momoSprite.setPosition(30, 90)
    haydenSprite.setPosition(70, 90)
    evanSprite.setPosition(110, 90)
    music.play(music.createSong(hex``), music.PlaybackMode.UntilDone)
    pause(1000)
    villianSprite = sprites.create(villian, SpriteKind.villain)
    pause(2000)
    game.showLongText("I hate music!!!", DialogLayout.Bottom)
    kalebSprite.setVelocity(40, 0)
    nandaniSprite.setVelocity(0, 40)
    hunterSprite.setVelocity(40, 20)
    momoSprite.setVelocity(10, 40)
    haydenSprite.setVelocity(60, 10)
    evanSprite.setVelocity(30, 60)
    pause(5000)
    villianSprite.setVelocity(50, 50)
    pause(1000)
    sprites.destroy(villianSprite)
    scene.setBackgroundImage(img`
        
        `)
    kalebSprite.setPosition(70, 80)
    game.showLongText("where am I?", DialogLayout.Right)
    pause(1000)
    sprites.destroy(kalebSprite)
    scene.setBackgroundImage(img`
        
        `)
    hunterSprite.setPosition(70, 80)
    game.showLongText("where am I?", DialogLayout.Top)
    pause(1000)
    sprites.destroy(hunterSprite)
    scene.setBackgroundImage(img`
        
        `)
    momoSprite.setPosition(70, 80)
    game.showLongText("where am I?", DialogLayout.Top)
    pause(1000)
    sprites.destroy(momoSprite)
    scene.setBackgroundImage(img`
        
        `)
    evanSprite.setPosition(70, 80)
    game.showLongText("where am I?", DialogLayout.Top)
    pause(1000)
    sprites.destroy(evanSprite)
    scene.setBackgroundImage(img`
        
        `)
    nandaniSprite.setPosition(70, 80)
    game.showLongText("where am I?", DialogLayout.Left)
    pause(1000)
    sprites.destroy(nandaniSprite)
    haydenSprite.setPosition(70, 80)
    scene.setBackgroundImage(img`
        
        `)
    game.showLongText("where am I?", DialogLayout.Right)
    pause(1000)
    sprites.destroy(haydenSprite)
}
function level2 () {
    if (previousLevel == 1 && currentLevel == 2) {
        game.setDialogCursor(img`
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            . . . . . . . b d b . . . . . . 
            . . . . . . . b d b c . . . . . 
            . . . . b b c 5 5 5 c b b . . . 
            . . . . b 5 5 5 1 5 5 5 b . . . 
            . . . c c 5 5 5 1 5 5 5 c c . . 
            . . b b 5 5 5 1 1 1 5 5 5 b b . 
            . . d d 5 1 1 1 1 1 1 1 5 d d . 
            . . b b 5 5 5 1 1 1 5 5 5 b b . 
            . . . c c 5 5 5 1 5 5 5 c c . . 
            . . . . b 5 5 5 1 5 5 5 b . . . 
            . . . . b b c 5 5 5 c b b . . . 
            . . . . . . c b d b c . . . . . 
            . . . . . . . b d b . . . . . . 
            . . . . . . . . . . . . . . . . 
            `)
        game.setDialogTextColor(1)
        game.setDialogFrame(img`
            ..eeeeeeeeeeeeeeeeeeee..
            .ee999999999999999999ee.
            ee999eeeeeeeeeeeeee999ee
            e999eeeeeeeeeeeeeeee999e
            e999eeeeeeeeeeeeeeee999e
            e999eeeeeeeeeeeeeeee999e
            e999eeeeeeeeeeeeeeee999e
            e999eeeeeeeeeeeeeeee999e
            e999eeeeeeeeeeeeeeee999e
            e999eeeeeeeeeeeeeeee999e
            e999eeeeeeeeeeeeeeee999e
            e999eeeeeeeeeeeeeeee999e
            e999eeeeeeeeeeeeeeee999e
            e999eeeeeeeeeeeeeeee999e
            e999eeeeeeeeeeeeeeee999e
            e999eeeeeeeeeeeeeeee999e
            e999eeeeeeeeeeeeeeee999e
            e999eeeeeeeeeeeeeeee999e
            e999eeeeeeeeeeeeeeee999e
            e999eeeeeeeeeeeeeeee999e
            e999eeeeeeeeeeeeeeee999e
            ee999eeeeeeeeeeeeee999ee
            .ee999999999999999999ee.
            ..eeeeeeeeeeeeeeeeeeee..
            `)
        info.setLife(3)
        scene.setBackgroundImage(img`
            
            fffffffffff3227d5b.
            `)
        scroller.scrollBackgroundWithSpeed(0, 60)
        myShip = sprites.create(assets.image`myImages`, SpriteKind.Player)
        controller.moveSprite(myShip, 100, 100)
        myShip.setStayInScreen(true)
        game.showLongText("Techno Guardian: The Silencer has stolen all Techno Music and put it on a rocket that is known to explode in flight. You must catch up with the rocket and recover the music!", DialogLayout.Bottom)
        game.showLongText("Techno Guardian: In order to recover the music, you have to destroy 5 purple rests. Press A to fire Projectile Notes. Make sure not to get hit by any Red Rests. They will cause you to lose a Life.", DialogLayout.Bottom)
    }
}
sprites.onOverlap(SpriteKind.Player, SpriteKind.Projectile, function (sprite, otherSprite) {
    if (currentLevel == 1) {
        sprites.destroy(triangle, effects.confetti, 100)
        sprites.destroy(violin, effects.confetti, 100)
        music.play(music.melodyPlayable(music.baDing), music.PlaybackMode.UntilDone)
        info.changeScoreBy(1)
    }
})
info.onScore(5, function () {
    if (currentLevel == 1) {
        music.stopAllSounds()
        game.showLongText("you saved classical music!", DialogLayout.Bottom)
        sprites.destroyAllSpritesOfKind(SpriteKind.Projectile)
        sprites.destroyAllSpritesOfKind(SpriteKind.Enemy)
        sprites.destroyAllSpritesOfKind(SpriteKind.Player)
        currentLevel = 2
        info.setScore(0)
    } else {
        game.showLongText("you saved techno music!", DialogLayout.Bottom)
        sprites.destroyAllSpritesOfKind(SpriteKind.Projectile)
        sprites.destroyAllSpritesOfKind(SpriteKind.Enemy)
        sprites.destroyAllSpritesOfKind(SpriteKind.Player)
        currentLevel = 3
    }
})
sprites.onOverlap(SpriteKind.Projectile, SpriteKind.Enemy, function (sprite, otherSprite) {
    if (currentLevel == 2) {
        sprite.destroy(effects.blizzard, 500)
        otherSprite.destroy(effects.blizzard, 500)
        info.changeScoreBy(1)
        otherSprite.sayText("Note Restored!")
    }
})
function gameover () {
    scene.setBackgroundImage(img`
        
        `)
    game.showLongText("YOU SAVED MUSIC", DialogLayout.Bottom)
}
sprites.onOverlap(SpriteKind.Player, SpriteKind.Enemy, function (sprite, otherSprite) {
    if (currentLevel == 1) {
        sprites.destroy(screw_driver, effects.disintegrate, 100)
        info.changeScoreBy(-1)
        music.play(music.createSong(hex``), music.PlaybackMode.UntilDone)
        sprites.destroy(screw_driver)
    }
    if (currentLevel == 2) {
        info.changeLifeBy(-1)
        scene.cameraShake(4, 500)
    }
})
let myRock: Sprite = null
let mySat: Sprite = null
let screw_driver: Sprite = null
let violin: Sprite = null
let triangle: Sprite = null
let villianSprite: Sprite = null
let evanSprite: Sprite = null
let haydenSprite: Sprite = null
let momoSprite: Sprite = null
let nandaniSprite: Sprite = null
let hunterSprite: Sprite = null
let myShip: Sprite = null
let projectile: Sprite = null
let kalebSprite: Sprite = null
let evan: Image = null
let hayden: Image = null
let kaleb: Image = null
let moselle: Image = null
let hunter: Image = null
let nandani: Image = null
let villian: Image = null
let currentLevel = 0
let previousLevel = 0
previousLevel = 0
currentLevel = 0
scene.setBackgroundImage(img`
    
    `)
villian = img`
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ...........................fffffffffff..........................
    ...........................ffffffffffff.........................
    ........................fffee22eee2eeefff.......................
    .......................ffeee222eee222eeeef......................
    ......................ffe222222222222222eef.....................
    .....................ffe22222eee22ee222e22ef....................
    ....................ffe2e222efff2fffe222e2eef...................
    ....................ff2ee222fffffffffe222e2ef...................
    ...................ffe2ee22febbebbbbbfe22e2eff..................
    ...................fe2ee22efd1dddddddfe22eeeeff.................
    ...................feee222eedddddddddee222eeeff.................
    ...................feee22efdddddddddd1ef22eeeff.................
    ...................feee22efedddddddddbff22eeeff.................
    ...................feef2eebe4dddddddbe4ee2eeeff.................
    ...................feef2febefedddddbffbef2ffeff.................
    ...................feef2efffffeddd4ffffff2feeff.................
    ...................ebbf2fcbf1cfdddef1fcbf2febee.................
    ...................ebdeffb1ecffdddefcfddffebdee.................
    ...................ebdeffb17ffedddbff7ddffeddee.................
    ...................ebbeefe177e3dddde77dbfeeb3ee.................
    ....................eeedeed13dddddddb1defdeee...................
    ....................fee33ddbdbeeeeeedddd3dbef...................
    ....................ffeeddddbbdddddbb4dddbeef...................
    .....................feee4ddecb111dcedd4eeeff...................
    ......................ffeeed4ffdd1cfedeeefff....................
    ......................f.fffeeb11111beeeff.ff....................
    ........................ff.efeeeeeeeee..f.......................
    ..........................fffed4e444fff.........................
    .........................ffffedbddbbffff........................
    ........................ffcfccffdcffccccf.......................
    ........................ffccfccfdcfcfcccf.......................
    .......................ffcccccffdcfccfccff......................
    .......................fcccfccffdcfccfcccf......................
    .......................fccffcccfdcfbccffcff.....................
    ......................fcccfccccfdcfcccffccff....................
    ......................ffcffccccfdcfcccffffff....................
    ......................fccffccbcfdcfbccffccff....................
    ......................fccffcccffdcfcccffccff....................
    ......................ffffffffffdcfffffffffc....................
    ......................ffcffccbcfffcbccffdbcf....................
    ......................ffccfffffffffffffcdbcf....................
    ......................ffccfffffffffffffcdcf.....................
    .......................ffffffffffffffffccf......................
    ........................fffffffffffffffff.......................
    ...........................fffff.ffffff.........................
    ...........................fcfff.fffcff.........................
    ...........................fbcff.ffcbff.........................
    ...........................fcbcf.fcccff.........................
    .........................fffffff.fffffff........................
    ........................ffffffff.ffffffff.......................
    ........................ffccccff.ffcccccf.......................
    ........................ffbccccf.ffcccbcf.......................
    ........................ffffffff.ffffffff.......................
    `
nandani = img`
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ...........................fffffffffff..........................
    .........................fffeeeeeeeeeffff.......................
    ........................fffee4eeee444effe.......................
    .......................feefe4444ee4444efef......................
    ......................feeee4e44eee44e4eeeeff....................
    .....................feeeee44444e44444eeeeeef...................
    ....................ffeeeee44444e44444eeeeeef...................
    ....................feeeee444444e444444eeeeef...................
    ...................feeeeee4e4444e444444eeeeeef..................
    ...................feeeeeeffffffffffffffeeeeef..................
    ...................feeeeffeeeeeeeeeeeeeeffeeef..................
    ...................feeeffe44444444444444effeef..................
    ...................feefe44eeeeeeeeeeeee44effef..................
    ...................fffeeeeffffffffffffffeeeeff..................
    ...................feeeefffffbbffccfffffffeeef..................
    ...................ffeefcbbbbcebfcffbbbbbffeff..................
    ...................ffffcd1111bcdfffb11111cffff..................
    ..................ffeef11dddd1dfffb1ddddd1cfeff.................
    ..................fe4efcbcbcbbcfffcbbccbcbfe4ef.................
    ..................fe3bffbbc1cbbfbfcbc1bcbcfedef.................
    .................ffedbfe1dfdcddfdfc1fdcc1bfedeff................
    ................fffe4bfed1bebdbfdecdbeed1efe3ecff...............
    ...............ffffffeefed1e1cf1d1cf1ed1efeeefffff..............
    ..................ffffebcffffcdddddbfffffb4efff.................
    ..................fffff4eeebed1ded1deeeee4effcf.................
    .................fffffffedd11dddeddd1ddd4effffff................
    .................fffffffeee33ddddddd3deeeffffffff...............
    ......................ffcffeeedddddbeeffcfff....................
    ......................fffffffeeeeeeeffffffff....................
    ......................ff.ffffebeee4bffff..ff....................
    ......................f...f444ddddd444ef..ff....................
    ..........................e554edddee55eb........................
    ..........................ee554edee55eeb........................
    .........................e4ee55e1ee5ee44e.......................
    ........................ee5eee44444eee55e.......................
    ........................b45e54e5e54e5e45e.......................
    .......................e454e455eee455e455e......................
    .......................e55e45555e45555e45e......................
    .......................e44e55555e55555ee5e......................
    .......................e44e55555e55555ee5eb.....................
    ......................ee54e55555e55555ee5eee....................
    ......................edeee55555e555554eedee....................
    ......................e3d4e44444e44444eeddee....................
    ......................eedbffcffcecffcffe1ee.....................
    .......................e4efffffffffffffe3e......................
    ........................eeefffffffffcfeee.......................
    ...........................fffff.fffcff.........................
    ...........................fffff.fffff..........................
    ...........................fffff.fffff..........................
    ...........................fffff.ffffff.........................
    ..........................ffffff.fffffff........................
    ........................ffeeeeff.ffeeeeff.......................
    ........................fcbbbeef.ffebbbef.......................
    ........................ffeeeeff.ffeeeecf.......................
    ........................ffffffff.ffffffff.......................
    `
hunter = img`
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    .....................................f..........................
    .....................................ff.........................
    ...........................fffffffffffff.f......................
    ........................ffffffeeeeffffffff......................
    .......................fffeeeeeeeeeeefffff......................
    ......................fffeeeeeeeeeeeeffffff.....................
    ......................ffeeeeeeeeeeeeeeeeffff....................
    ......................ffeeeeeeeeeeeeeeeeefff....................
    ....................fffeeeeeeeeeeeeeeeeeeefff...................
    ....................fffeeeeeffeeeeeeefeeeefff...................
    ....................fffeeeefffeeeeeeeffeeefff...................
    ....................fffeeeffeeeeeeeeefffeefff...................
    ...................fffeeeffeeeeeeeeefffffefff...................
    .................fffeefeffeeeeeeeeeffffefffff...................
    ..................fffffffeeeeeeeeffffed4fffff...................
    ...................ffffffffffffffffcfbbb4ffff...................
    ...................feeffffffffffffedfffedffe4ee.................
    ...................e4efdfcbf1cfdddef1fcbfdfc3ee.................
    ...................ebbedeb1ecffdddefcfddfdeedee.................
    ...................ebded4d17ffedddbff7d14debdee.................
    ...................ebbe3d11777bdddde77d1ddeebee.................
    ....................eeeb3d111ddddddd111ddbeee...................
    ......................ebddd1ddddddddd11dddee....................
    ......................eebddddddddddddddd3ee.....................
    .......................eebbddddddddddd3bee......................
    ........................eeed3ddddddd3deee.......................
    ...........................eeedddddbeee.........................
    .............................eeeeeee............................
    ..........................f..f73437f..f.........................
    .........................fffffc3bdefffff........................
    ........................ffccbcffffffbcccf.......................
    ........................ffcccbcf7fcbccccf.......................
    .......................ffccfcccf7ffccfccff......................
    .......................fcccfcccf7ffccfcccf......................
    .......................fccffcccf7ffcccffcff.....................
    ......................fcccfccccf7ffcccffccff....................
    ......................fccffccccf7ffcccffccff....................
    ......................fbbcfccbcf7ffbccffbbff....................
    ......................fccffccccf7ffcccffccff....................
    ......................effffccccf7ffcccffffee....................
    ......................ed1efcbbcf7fcbbbfcddee....................
    ......................e31bffffff7ffffffc1dee....................
    ......................eedbfffff868fffffcdee.....................
    .......................e4bf86888888868fc4e......................
    ........................eeef888ffff88feee.......................
    ...........................f888f.f888ff.........................
    ...........................f888f.f888ff.........................
    ...........................f868f.f886ff.........................
    ...........................f888f.ff88ff.........................
    .........................fffffff.fffffff........................
    ........................eebdddbe.eebddbbe.......................
    ........................eb1111de.e41111de.......................
    ........................ebddddbe.eeddddbe.......................
    ........................eeeeeeee.eeeeeeee.......................
    `
moselle = img`
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ...........................fffffffffff..........................
    ...........................ffffffffffff.........................
    ........................fffeeeeeeeeeeefff.......................
    .......................ffeeeeeeeeeeeeeeeff......................
    ......................ffeeeeeeeeeeeeeeeeeff.....................
    .....................ffeeeeeeeeeeeeeeeeeeeff....................
    ....................fffeffeeeeeeeeeeeeeefeeff...................
    ....................ffeeeebefeeefeeefebefeecf...................
    ...................fffefeeeefcbefebefeeeefeeff..................
    ...................ffeefeeeefeefcfeeffeeeffeff..................
    ...................ffffeeefffcffdcfefffeeeffff..................
    ..................fffffeefffeffdddefefffeeefff..................
    ..................f.fffeffeffedddddbffeefeeff...................
    ..................f.ffffffffffe1d1bffffffffff...................
    ....................effccccccccbebccccccccfff...................
    ...................eeef1111111bfffc1111111ff4e..................
    ...................e4efdbbdc1dcfdffb1bbdc1fc4e..................
    ...................e4bf1bb1bbbffdcfbbeddb1fede..................
    ...................eddfcd11dbbcfdffdb411dcfbde..................
    ...................eb3efcddeebccdecbeebdbfe4be..................
    ....................eeedcfffffc1d1bffffffdeee...................
    ....................ffe33bddddddddddddddbd4ef...................
    .....................ffe3d11bd1ded1d4d1dd4ef....................
    ......................ffe43debdbebdbedd4eeff....................
    ......................ffeeeddbeedbeeddeeefff....................
    ...................ffffffffeebdddddbeeffffffff..................
    ..................fffffffffffeeeeeeefffffffffff.................
    ..................ffeefffffffed4e43bffff.ffeeff.................
    .................fffeeff..fffebbddbbffff.ffeeeff................
    .................ffeefffffccbcffdcffbcccfffffeeff...............
    ..............f..ffeeff.ffcccbcfffcbccccf.ffefeff.ff............
    ...............fffffff.ffccfcccfffcccfccff.ffffffff.............
    ...............ffffff..fcccfcccfeffccfcccf..ffffff..............
    .......................fccffcccf5efcccffcff.....................
    ......................fcccfccccf5ffcccffccff....................
    ......................fccffccccf5efcccffccff....................
    ......................fcbcfccbcfeffbccffbbff....................
    ......................fccffccccffffcccffccff....................
    ......................effffccccffffcccffffee....................
    ......................eddefcbbcfffcbbbfcddee....................
    ......................e31bfffffffffffffc1dee....................
    ......................eedbfffffccccffffcdee.....................
    .......................e4bf86686666668fc4e......................
    ........................eeef868fff866feee.......................
    ...........................f668f.f866ff.........................
    ...........................f668f.f866ff.........................
    ...........................f668f.f866ff.........................
    ...........................f888f.ff86ff.........................
    .........................fffffff.fffffff........................
    ........................66699966.66999966.......................
    ........................66999996.66999996.......................
    ........................66999966.66999966.......................
    ........................66666666.66666666.......................
    `
kaleb = img`
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ........................ff...ff......ff.........................
    ........................ff..feef....fee...ff....................
    ........................ee..f44f...fe4ef..ff....................
    .......................f44fffe4eeffe44ef.eff....................
    ....................ff.fe44efe444fe444eff4ef....................
    ....................fffffe444ee44ee44eee44ef..ff................
    ....................feeefeee4ee44444ee444effffff................
    ....................fe444eeeee44444ee444eeeeeeff................
    ....................ffe444444444e44444ee444444ef................
    ...................feefe444444ee44444e444eeeef..................
    ...................feeeffeeeeeee444444e444eeff..................
    ...................fe44efeeeefe444ee44ee444eeef.................
    .................ffe444e4444efe44eeee44ee444eeff................
    .................ffe4ef4444fdee44fbdfe4ef444e4ef................
    .................ffeefe45efeeee4fb1def44fe44e4ef................
    .................ffeff44eebddef4fbdedfe4ff4efeff................
    .................ffffe44eebbbeeeeb3ebfeeef4effff................
    .................ffeee4fbbfffb3fdddbfffedfeefeff................
    ...................efe4ffb1fbcffddef1fcbfdeebe..................
    ...................ebefbefc71cf4ddefce1dfdeede..................
    ...................eddfddef71cf1ddbff7114deede..................
    ...................ebbe3dd771d4dddd4d7d1ddebbe..................
    ....................eeeb3d111ddddddd111ddbeee...................
    ......................ebddd11beeeeebd11dddee....................
    ......................eebddd1eedd1eedddd3ee.....................
    .......................eebbddbbdddbbdd3bee......................
    ........................eeed3ddeeeb13deee.......................
    ...........................eeedddddbeee.........................
    .............................eeeeeee............................
    ..........................e..ebb44be..e.........................
    .........................e2e2eb3d3be2e2e........................
    ........................e22222eebee22222e.......................
    ........................e222222e1e222222e.......................
    .......................e2222222e1ee222222e......................
    .......................e2222222e1ee222222e......................
    .......................e2222222e1e22222222e.....................
    ......................e222e2222e1e22222e222e....................
    ......................e222e2222e1e22222e22ee....................
    ......................e222e2222e1e22222e222e....................
    ......................e222e2222e1e22222e22ee....................
    ......................eeeee2222e1ee2222eeeee....................
    ......................ed1be2222e1e22222eddee....................
    ......................ed1b22222ede22222e1dee....................
    ......................eedbfffffecefffffedee.....................
    .......................e4efffffffffffffc3e......................
    ........................eeefffffffffffeee.......................
    ...........................fffff.ffffff.........................
    ...........................fffff.ffffff.........................
    ...........................ccbcc.ccbbcc.........................
    ...........................b11db.bd11dc.........................
    .........................fffffff.fffffff........................
    ........................ffffffff.ffffffff.......................
    ........................ffccccff.ffccccff.......................
    ........................ffcfffff.fffffcff.......................
    ........................ffffffff.ffffffff.......................
    `
hayden = img`
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ...........................fffff.fffff..........................
    ...........................fffff.ffffff.........................
    ........................fffeeeeefeeeeefff.......................
    .......................ffeeeeeeefeeeeeeeef......................
    ......................ffe44eeeeeeeeeeeeeeef.....................
    .....................ffee44eeeeeeeeeeeeeeeef....................
    ....................ffee44eeeeeee44eeeeeeeeef...................
    ....................ffeeeeee44efe44eeeeeeeeef...................
    ...................ffeeeeee4eeefe44eefe4eeeeff..................
    ...................ffeeefe4eefffe4eeefeeeeeeeff.................
    ...................feeeeffeeefffe4eeeffeefeeeff.................
    ...................ffeeeffeefbdfeeeeffeeefeeeff.................
    ...................ffeeffeeffb1bfeefeeffefeeeff.................
    ...................ffeefffebd4bdfeefdd3efffeeff.................
    ...................ffeeefffdbe4defeebbdefefeeff.................
    ...................ffffdeffffedddfedfffffdcfeff.................
    ...................eeffdecbf1bfdddef1fcbfdeffee.................
    ...................efffdeb1ecffdddefcfddfdeffee.................
    ...................eeefd4d17ffedddbff7d14dffeee.................
    ...................edbfbd11777bdddde77d1ddfedee.................
    ....................eeeb3d111ddddddd111ddbeee...................
    ......................ebddd1ddddddddd11dddee....................
    ......................eebddddddddddddddd3ee.....................
    .......................eebbddddddddddd3bee......................
    ........................eee33ddddddd3deee.......................
    ...........................eeedddddbeee.........................
    ...........................c8e4eeeee8c8.........................
    ..........................8668ebb4ec668.........................
    .........................88696c334c89668........................
    ........................866668ceeec866668.......................
    ........................869666c222e866668.......................
    .......................8869666c222e8666668......................
    .......................8666666c222e8666668......................
    .......................8668666c222e86666688.....................
    ......................86968666c222e866889688....................
    ......................86688696c222e866886688....................
    ......................86968696c222e896886688....................
    ......................c6688666c222e86688666c....................
    ......................e8888668c222e8668888ee....................
    ......................eddb8696c222e8968cddee....................
    ......................e3db8888c2e2c88886ddee....................
    ......................eedb8666bbbbb6668bdee.....................
    .......................e4bcd1d99999d1dbb4e......................
    ........................eeec99bcccb996eee.......................
    ...........................699bc.cb996c.........................
    ...........................c99bc.cb99b6.........................
    ...........................c91bc.cb91bc.........................
    ...........................c99bc.cb99bc.........................
    .........................c6ccc66.66ccc66........................
    ........................ffcccccf.fcccbccf.......................
    ........................ffbbbbcf.ffbbbbcf.......................
    ........................ffccccff.ffcccccf.......................
    ........................ffffffff.ffffffff.......................
    `
evan = img`
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ............................fffffffff...........................
    ............................fffffffff...........................
    ........................ffffeeeeeeeeeffff.......................
    .......................ffeee4444444e4eeeef......................
    ......................ffee4444e4e44e444eeef.....................
    .....................ffeee44ee4e4ee4e44eeeef....................
    ....................ffeeeee4e44e4444e44eeeeef...................
    ....................ffe44eeeeeeeeeeeeeeee4eef...................
    ...................ffeeee4ee4ee4e4ee4ee4eeeeff..................
    ...................feeeee4ef4ee4f4ef4fe44eeeeff.................
    ...................ffeee44fceeeefeeeeefe4eeeeff.................
    ...................ffeefeef1fbdfdeedfdef4feeeff.................
    ...................feeefffeeebddddddeeeeffeeeff.................
    ...................feefefed4d4bdddbed4dbfeeeeff.................
    ...................feefddbbbbe4ddd4ebbbb4dcfeff.................
    ...................feef3defffedddddbfffcddeeeff.................
    ...................ebbedfcbf1bfdddef1fcbfdeebee.................
    ...................ebdedec1ecffdddefcfddfdebdee.................
    ...................ebded4d17ffedddbff7d14debdee.................
    ...................ebbe3d11777bdddd477d1ddeebee.................
    ....................eeeb3d111ddddddd111ddbeee...................
    ......................ebddd11beeeeebd11dddee....................
    ......................eebddd1eedd1eedddd3ee.....................
    .......................eebbddbbdddbbdd3bee......................
    ........................eeed3ddeeeb13deee.......................
    ...........................eeedddddbeee.........................
    .............................eeeeeee............................
    ..........................f..cb444bc..f.........................
    .........................fffff63d3b8ffff........................
    ........................ffeeeeff6fffeeeef.......................
    ........................ffeefeef98feeeeef.......................
    .......................ffeeeeeff98feeeeeff......................
    .......................feeefeeff9cfeefeeef......................
    .......................feefeeeef9ce2eefeeff.....................
    ......................fe2efeeeef9cfeeeffeeff....................
    ......................ffeffeeeef9cfeeeffeeff....................
    ......................fe2efee2ef9cf2eeffeeff....................
    ......................feeffeeeef9cfeeeffeeff....................
    ......................effffeeeff6cfeeeffffee....................
    ......................ed1bfee2ef98f2eefcddee....................
    ......................e31bffffff6ffffffc1dee....................
    ......................eedbcccccb6bccccccdee.....................
    .......................e4b691199d99919bb4e......................
    ........................eeec99bcccb996eee.......................
    ...........................699bc.cb996c.........................
    ...........................c99bc.cb99b6.........................
    ...........................c91bc.cb91bc.........................
    ...........................c99bc.cb99bc.........................
    .........................c6ccc66.66ccc66........................
    ........................ffcccccf.fcccbccf.......................
    ........................ffbbbbcf.ffbbbbcf.......................
    ........................ffccccff.ffcccccf.......................
    ........................ffffffff.ffffffff.......................
    `
game.onUpdateInterval(5000, function () {
    if (currentLevel == 1) {
        triangle = sprites.createProjectileFromSide(img`
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            . . . . . . . 4 . . . . 4 4 4 . 
            . . . . . . 4 4 4 . . . 4 4 4 . 
            . . . . . 4 4 . 4 4 . . . 4 4 4 
            . . . . 4 4 . . . 4 4 . . . 4 4 
            . . . 4 4 . . . . . 4 4 . . . 4 
            . . 4 4 . . . . . . . 4 4 . . . 
            . 4 4 . . . . . . . . . 4 4 . . 
            4 4 4 4 4 4 4 4 4 4 4 4 4 4 4 . 
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            `, randint(-50, 50), randint(-50, 50))
        violin = sprites.createProjectileFromSide(img`
            .eeeee.f....f.............
            .eeeeee.f..f.ff...........
            .eeeefee...1...ff.........
            .eefeefe....1....f........
            .eeefeef.....11...ff......
            .eeeffeef......1....f.....
            f.eeeffeef......11...ff...
            .f...efffef.......11...ff.
            ......efefef........1...ff
            .......efefeff.......11.ff
            ........efefeef........1ff
            ..........fefeef..........
            ...........fffeefe........
            ............effeef........
            .............efffffefee...
            ..............efffffeffee.
            ..............eeffff4eefee
            ..............efefe4efeeee
            ..............eefe4efefeee
            ..............eefeefefffee
            ..............eeefeefffeee
            ..............eeeeeeefeeee
            ..............eeeeeeeeeeee
            ...............eeeeeeeeeee
            ...................eeeeee.
            ..........................
            `, randint(-50, 50), randint(-50, 50))
        screw_driver = sprites.createProjectileFromSide(img`
            ..........................
            ..........................
            ..................11111111
            ......................1111
            ......11..............1111
            .....1444.............1111
            ....11.444............1111
            ...1fff1444...........1111
            ..1ff1f114444.........1111
            .1fffff1114444........1111
            1fff111111114444......1111
            ..1f1fff111114444.....1111
            ..1f1f1f1ff1114444....1111
            ...11ffff1f111114444..1111
            ....1111f1f111111144111111
            .....1fff11f11111111......
            .....11111111111111.......
            ......11ff1f111111........
            .......1f1ff1f111.........
            ........1f1f1f11..........
            ........111ff11...........
            .........11111............
            ..........111.............
            ..........................
            ..........................
            ..........................
            `, randint(-50, 50), randint(-50, 50))
        screw_driver.setKind(SpriteKind.Enemy)
    }
})
game.onUpdateInterval(1500, function () {
    if (currentLevel == 2) {
        mySat = sprites.createProjectileFromSide(img`
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            . . . . . . a a a a a a . . . . 
            . . . . . a 1 1 1 1 1 1 a . . . 
            . . . . . a a a a a 1 a a . . . 
            . . . . . a a a a 1 a a a . . . 
            . . . . . a a a 1 a a a a . . . 
            . . . . . a a 1 a a a a a . . . 
            . . . . . a 1 1 1 1 1 1 a . . . 
            . . . . . a 1 a a a a a a . . . 
            . . . . . a 1 a a a a a a . . . 
            . . . . . a 1 a a a a a a . . . 
            . . . . . a a 1 1 1 1 1 a . . . 
            . . . . . . a a a a a a . . . . 
            . . . . . . . . . . . . . . . . 
            `, 0, 73)
        mySat.setKind(SpriteKind.Enemy)
        mySat.x = randint(5, 155)
        myRock = sprites.createProjectileFromSide(img`
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            . . . . . . 3 3 3 3 3 3 . . . . 
            . . . . . 3 1 1 1 1 1 1 3 . . . 
            . . . . . 3 3 3 3 3 1 3 3 . . . 
            . . . . . 3 3 3 3 1 3 3 3 . . . 
            . . . . . 3 3 3 1 3 3 3 3 . . . 
            . . . . . 3 3 1 3 3 3 3 3 . . . 
            . . . . . 3 1 1 1 1 1 1 3 . . . 
            . . . . . 3 1 3 3 3 3 3 3 . . . 
            . . . . . 3 1 3 3 3 3 3 3 . . . 
            . . . . . 3 1 3 3 3 3 3 3 . . . 
            . . . . . 3 3 1 1 1 1 1 3 . . . 
            . . . . . . 3 3 3 3 3 3 . . . . 
            . . . . . . . . . . . . . . . . 
            `, 0, 100)
        myRock.x = randint(5, 155)
        myRock.setKind(SpriteKind.Enemy)
    }
})
forever(function () {
    if (currentLevel == 0) {
        intro()
        currentLevel = 1
        previousLevel = 0
    } else if (currentLevel == 1) {
        level1()
        previousLevel = 1
    } else if (currentLevel == 2) {
        level2()
        previousLevel = 2
    } else {
        gameover()
    }
})
